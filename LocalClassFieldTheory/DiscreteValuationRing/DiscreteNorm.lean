  ...
  rw [_root_.map_pow, ← NNReal.rpow_nat_cast,
    Nat.cast_div (minpoly.degree_dvd (is_algebraic_iff_is_integral.mp (h_alg ↑x)))
      (nat.cast_ne_zero.mpr
        (ne_of_gt (minpoly.natDegree_pos (is_algebraic_iff_is_integral.mp (h_alg ↑x))))),
...
def nontriviallyDiscretelyNormedField : NontriviallyNormedField K :=
  { @RankOneValuation.ValuedField.toNormedField K _ ℤₘ₀ _ _ (DiscreteValuation.isRankOne _) with
    non_trivial := by
      ...
      use x.1⁻¹
      erw [@norm_inv K (@NormedField.toNormedDivisionRing K (discretely_normed_field K)),
...
def discretelySemiNormedCommRing : SeminormedCommRing K :=
  by
  letI : NontriviallyNormedField K := nontrivially_discretely_normed_field K
  infer_instance

/-- The seminormed ring structure on `K` induced by its discrete valuation. -/
def discretelySemiNormedRing : SeminormedRing K :=
  by
  letI : NontriviallyNormedField K := nontrivially_discretely_normed_field K
  infer_instance
...
variable {K} [CompleteSpace K] {L : Type _} [Field L] [Algebra K L]
...
def discretelyNormedFieldExtensionUniformSpace (h_alg : Algebra.IsAlgebraic K L) : UniformSpace L :=
  by haveI := discretely_normed_field_extension h_alg <;> infer_instance
...
theorem hMul (h_alg : Algebra.IsAlgebraic K L) (x y : L) :
  ...
  @spectral_norm_is_hMul K (nontriviallyDiscretelyNormedField K) L _ _ h_alg _
    (norm_isNonarchimedean K) x y

theorem le_one_iff_integral_minpoly (h_alg : Algebra.IsAlgebraic K L) (x : L) :
    discreteNormExtension h_alg x ≤ 1 ↔ ∀ n : ℕ, hv.V ((minpoly K x).coeff n) ≤ 1 :=
  by
  letI := nontrivially_discretely_normed_field K
  have h : spectralMulAlgNorm h_alg _ x = spectralNorm K L x; rfl
  rw [discrete_norm_extension, h, spectralNorm,
    spectralValue_le_one_iff (minpoly.monic (is_algebraic_iff_is_integral.mp (h_alg x)))]
...
theorem of_integer [fr : IsFractionRing hv.V.ValuationSubring.toSubring K]
  ...
  by
  letI := nontrivially_discretely_normed_field K
  letI : ValuationRing hv.v.valuation_subring.to_subring := hv.v.valuation_subring.valuation_ring
  have is_minpoly :
    minpoly K (x : L) =
      Polynomial.map (algebraMap hv.v.valuation_subring.to_subring K)
        (minpoly hv.v.valuation_subring.to_subring x) :=
    by
    rw [eq_comm]
    exact minpoly_of_subring K L hv.v.valuation_subring.to_subring x
  have h_app : (spectralMulAlgNorm h_alg _) ↑x = spectralNorm K L (x : L) := rfl
  rw [discrete_norm_extension, h_app, spectralNorm, ← is_minpoly]
  all_goals exact norm_is_nonarchimedean K

theorem le_one_of_integer [fr : IsFractionRing hv.V.ValuationSubring.toSubring K]
    (h_alg : Algebra.IsAlgebraic K L) (x : integralClosure hv.V.ValuationSubring.toSubring L) :
    discreteNormExtension h_alg x ≤ 1 :=
  by
  letI := nontrivially_discretely_normed_field K
  letI : ValuationRing hv.v.valuation_subring.to_subring := hv.v.valuation_subring.valuation_ring
