...
--instance : Algebra ↥K₀ ↥(integralClosure ↥K₀ L) := inferInstance
...
--Porting note: In Lean3 the following was already found as an instance, now it has to be specified
--instance : SMul ↥K₀ (integralClosure ↥K₀ L) := inferInstance --Algebra.toSMul

--Porting note: In Lean3 the following was already found as an instance, now it has to be specified
--instance : MulZeroClass ↥(integralClosure ↥K₀ L) := inferInstance --MulZeroOneClass.toMulZeroClass

instance instNoZeroSMulDivisors : NoZeroSMulDivisors K₀ (integralClosure K₀ L)
...
def powExtensionOnUnits [FiniteDimensional K L] : Lˣ →* Multiplicative ℤ where
  ...
  map_one' := by simp [Units.val_one, minpoly.one, coeff_sub, coeff_X_zero, coeff_one_zero,
    zero_sub, Valuation.map_neg, _root_.map_one, one_pow, unzero_coe, One.one]
  map_mul' x y := by
    simp only [Units.val_mul]
    rw [← WithZero.coe_inj, WithZero.coe_mul, WithZero.coe_unzero, WithZero.coe_unzero,
      WithZero.coe_unzero, map_mul_aux x y]

theorem powExtensionOnUnits_apply [FiniteDimensional K L] (x : Lˣ) :
    powExtensionOnUnits K L x = WithZero.unzero
      (Valuation.pow_coeff_zero_ne_zero_of_unit hv.v x.1 x.isUnit) := rfl
...
theorem expExtensionOnUnits_ne_zero [FiniteDimensional K L] : expExtensionOnUnits K L ≠ 0 := by
  ...
  have hzne1 : powExtensionOnUnits K L z ≠ 1 := by
    have hv :  Valued.v ((minpoly K ((Units.map (algebraMap K L).toMonoidHom)
        hx_unit.unit).val).coeff 0) = Valued.v (x : K) := by
...
/-- The image of `powExtensionOnUnits K L` is generated by `ofAdd (expExtensionOnUnits K L : ℤ)`. -/
theorem expExtensionOnUnits_generates_range [FiniteDimensional K L] :
...
theorem exists_mul_expExtensionOnUnits [FiniteDimensional K L] (x : Lˣ) :
    ∃ n : ℤ, ((ofAdd (-1 : ℤ) ^ n) ^ expExtensionOnUnits K L : ℤₘ₀) =
      Valued.v ((minpoly K (x : L)).coeff 0) ^ (finrank K L / (minpoly K (x : L)).natDegree) := by
  set y := WithZero.unzero  (Valuation.pow_coeff_zero_ne_zero_of_unit hv.v x.1 x.isUnit)
  have h_mem : WithZero.unzero  (Valuation.pow_coeff_zero_ne_zero_of_unit hv.v x.1 x.isUnit) ∈
      Subgroup.closure ({ofAdd (expExtensionOnUnits K L : ℤ)} : Set (Multiplicative ℤ)) := by
    rw [← expExtensionOnUnits_generates_range, Subgroup.mem_map]
    exact ⟨x, mem_top x, rfl⟩
  obtain ⟨n, hn⟩ := Subgroup.mem_closure_singleton.mp h_mem
  use -n
...
theorem expExtensionOnUnits_dvd [FiniteDimensional K L] :
  ...
  have hn0 : 0 ≤ n :=
    nonneg_of_mul_nonneg_left (by simp only [hn, Nat.cast_nonneg])
...
def extensionDef [FiniteDimensional K L] : L → ℤₘ₀ := fun x ↦
...
theorem extensionDef_apply [FiniteDimensional K L] (x : L) :
    extensionDef K x = if hx : x = 0 then (0 : ℤₘ₀) else ofAdd (-1 : ℤ) ^
      (exists_mul_expExtensionOnUnits K (isUnit_iff_ne_zero.mpr hx).choose).choose := rfl
    ...
    · by_cases hxy : x + y = 0
      ...
      · simp only [extensionDef_apply, dif_neg hx, dif_neg hy, dif_neg hxy]
        ...
        rw [_root_.map_pow, NNReal.coe_pow, ← Real.rpow_natCast, Nat.cast_div,
          ← pow_eq_pow_root_zero_coeff'] --x + y
        rw [_root_.map_pow, NNReal.coe_pow, ← Real.rpow_natCast _
          (finrank K L / (minpoly K _).natDegree), Nat.cast_div,
          ← pow_eq_pow_root_zero_coeff'] -- x
        rw [_root_.map_pow, NNReal.coe_pow, ← Real.rpow_natCast _
          (finrank K L / (minpoly K _).natDegree), Nat.cast_div,
          ← pow_eq_pow_root_zero_coeff'] -- y
        have h_le : (discreteNormExtension (K := K)) (x + y) ≤ (discreteNormExtension (K := K)) x ∨
            (discreteNormExtension (K := K)) (x + y) ≤ (discreteNormExtension (K := K)) y :=
           _root_.le_max_iff.mp ((isNonarchimedean) _ _)
        rcases h_le with hlex | hley
        · exact Or.inl (pow_le_pow_left₀ (nonneg _) hlex _)
        · exact Or.inr (pow_le_pow_left₀ (nonneg _) hley _)
...
def extendedValuation [FiniteDimensional K L] : Valuation L ℤₘ₀ where
...
theorem apply [FiniteDimensional K L] (x : L) :
    extendedValuation K L x = if hx : x = 0 then (0 : ℤₘ₀)  else ofAdd (-1 : ℤ) ^
      (exists_mul_expExtensionOnUnits K (isUnit_iff_ne_zero.mpr hx).choose).choose := rfl

theorem apply_if_neg [FiniteDimensional K L] {x : L} (hx : x ≠ 0) :
    extendedValuation K L x = ofAdd (-1 : ℤ) ^
      (exists_mul_expExtensionOnUnits K (isUnit_iff_ne_zero.mpr hx).choose).choose := by
  rw [apply, dif_neg hx]
...
--instance :  Add (integralClosure hv.v.valuationSubring L) := inferInstance
...
instance : IsValExtension hv.v (extendedValuation K L) := by
  apply IsValExtension.ofComapInteger
  ext x
  simp only [Subring.mem_comap, Valuation.mem_integer_iff,
    le_one_iff_discreteNormExtension_le_one, discreteNormExtension, spectralMulAlgNorm_def,
