...
* `discrete_valuation.powExtensionOnUnits` : The map sending `x : Lˣ` to
...
instance (priority := 100) completeSpace [FiniteDimensional K L] :
  ...
  letI : NontriviallyNormedField K := nontrivially_discretelyNormedField K
  spectral_norm_completeSpace (Algebra.IsAlgebraic.of_finite K L) (norm_is_nonarchimedean K)
...
def ValuationSubring.algebra : Algebra K₀ L₀ :=
  haveI h : Algebra hv.v.valuation_subring (extendedValuation K L).valuationSubring.toSubring :=
    by
