...
* `DiscreteValuation.exp_extension_on_units` : the natural number `n` such that `of_add (n : ℤ)`
...
* `DiscreteValuation.exp_extension_on_units_generates_range` : The image of
 `powExtensionOnUnits K L` is  generated by `of_add (exp_extension_on_units K L : ℤ)`.
* `DiscreteValuation.exp_extension_on_units_dvd` : The number `exp_extension_on_units K L`
...
theorem map_hMul_aux [FiniteDimensional K L] (x y : Lˣ) :
  ...
  have hinj : Injective (withZeroMultIntToNnreal (base_ne_zero K hv.v)) :=
    sorry--(withZeroMultIntToNnreal_strictMono (one_lt_base K hv.v)).Injective
  ...
  · by_cases hy : y = 0
    ...
    · by_cases hxy : x + y = 0
      ...
      · simp only [extensionDef_apply, dif_neg hx, dif_neg hy, dif_neg hxy]
        set ux := (exists_mul_expExtensionOnUnits K (isUnit_iff_ne_zero.mpr hx).choose).choose with
          hux_def
        set uy := (exists_mul_expExtensionOnUnits K (isUnit_iff_ne_zero.mpr hy).choose).choose with
          huy_def
        set uxy :=
          (exists_mul_expExtensionOnUnits K (isUnit_iff_ne_zero.mpr hxy).choose).choose with
          huxy_def
        ...
        have hd : 0 < (expExtensionOnUnits K L : ℤ) := by
          rw [Int.coe_nat_pos]
          exact Nat.pos_of_ne_zero expExtensionOnUnits_ne_zero
        sorry
...
theorem extensionDef_one [FiniteDimensional K L] : extensionDef K (1 : L) = 1 := by
  ...
  have hu :
    (↑(ofAdd (-1 : ℤ)) ^ u) ^ expExtensionOnUnits K L =
      Valued.v ((minpoly K ↑(isUnit_iff_ne_zero.mpr h1).choose).coeff 0) ^
        (finrank K L / (minpoly K ((isUnit_iff_ne_zero.mpr h1).choose : L)).natDegree) :=
    sorry
    -- (exists_mul_expExtensionOnUnits K (is_unit_iff_ne_zero.mpr h1).Unit).choose_spec
...
theorem le_one_iff_discreteNormExtension_le_one [FiniteDimensional K L] (x : L) :
  ...
  · have h_le_iff :
      discreteNormExtension h_alg x ≤ 1 ↔ discreteNormExtension h_alg x ^ finrank K L ≤ 1 :=
      by
    ...
    rw [/- ← hn_def, -/ h_le_iff,
      pow_eq_pow_root_zero_coeff _ _
        (minpoly.degree_dvd (isAlgebraic_iff_isIntegral.mp (h_alg x))),
      ← NNReal.coe_pow, ← _root_.map_pow]
    -- erw [← (exists_mul_expExtensionOnUnits K (isUnit_iff_ne_zero.mpr hx).choose).choose_spec]
  ...
  have hπ1 : extendedValuation K L x = Multiplicative.ofAdd (-1 : ℤ) := by
    rw [Extension.apply_if_neg, ←
      WithZero.zpow_left_inj _ WithZero.coe_ne_zero
        (Nat.cast_ne_zero.mpr _)]
    sorry
    · have hx0 : (x : L) ≠ 0 := Units.ne_zero _
      rw [zpow_ofNat, zpow_ofNat, ← hx]
      all_goals{sorry}
      -- erw [(exists_mul_expExtensionOnUnits K x).choose_spec]
      -- rfl
    · sorry
...
theorem isComplete [FiniteDimensional K L] :
    @IsComplete L (UniformSpace (Algebra.IsAlgebraic.of_finite K L)) Set.univ :=
  by
...
def ValuationSubring.algebra : Algebra K₀ L₀ :=
  haveI h : Algebra hv.v.valuationSubring (extendedValuation K L).valuationSubring.toSubring :=
    by
