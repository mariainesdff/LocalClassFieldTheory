...
theorem Real.exists_isGLB {S : Set ℝ} (hne : S.Nonempty) (hbdd : BddBelow S) : ∃ x, IsGLB S x :=
  by
  set T := -S with hT
  have hT_ne : T.nonempty := set.nonempty_neg.mpr hne
  have hT_bdd : BddAbove T := bdd_above_neg.mpr hbdd
  use-Classical.choose (Real.exists_isLUB T hT_ne hT_bdd)
  simpa [← isLUB_neg] using Classical.choose_spec (Real.exists_isLUB T hT_ne hT_bdd)
...
theorem Filter.tendsto_of_is_bounded_monotone {f : ℕ → ℝ} (h_bdd : BddAbove (Set.range f))
    (h_mon : Monotone f) : ∃ r : ℝ, Filter.Tendsto f Filter.atTop (𝓝 r) :=
  by
...
theorem Real.tendsto_of_is_bounded_antitone {f : ℕ → ℝ} (h_bdd : BddBelow (Set.range f))
    (h_ant : Antitone f) : ∃ r : ℝ, Filter.Tendsto f Filter.atTop (𝓝 r) :=
  by
...
theorem NNReal.tendsto_of_is_bounded_antitone {f : ℕ → NNReal} (h_ant : Antitone f) :
    ∃ r : NNReal, Filter.Tendsto f Filter.atTop (𝓝 r) :=
  by
