...
theorem seminormFromConstSeq_nonneg (x : R) (n : ℕ) : 0 ≤ seminormFromConstSeq c f x n :=
...
theorem seminorm_from_const_is_bounded (x : R) :
    BddBelow (Set.range (seminormFromConstSeq c f x)) :=
  by
  use 0
  rw [mem_lowerBounds]
  intro r hr
  obtain ⟨n, hn⟩ := hr
  rw [← hn]
  exact seminormFromConstSeq_nonneg c f x n

variable {f}

/-- `seminorm_from_const_seq c f 0` is the constant sequence zero. -/
theorem seminormFromConstSeq_zero (hf : f 0 = 0) : seminormFromConstSeq c f 0 = 0 :=
  by
...
theorem seminormFromConstSeq_one (n : ℕ) (hn : 1 ≤ n) : seminormFromConstSeq c f 1 n = 1 :=
  by
  simp only [seminormFromConstSeq]
  rw [one_mul, hpm _ hn, div_self (pow_ne_zero n (Ne.symm hc))]

/-- `seminorm_from_const_seq c f x` is antitone. -/
theorem seminormFromConstSeq_antitone (x : R) : Antitone (seminormFromConstSeq c f x) :=
  by
  intro m n hmn
  simp only [seminormFromConstSeq]
...
theorem seminorm_from_const_zero : seminormFromConst' hf1 hc hpm 0 = 0 :=
  tendsto_nhds_unique (seminorm_from_const_is_limit hf1 hc hpm 0)
    (by simpa [seminormFromConstSeq_zero c (map_zero _)] using tendsto_const_nhds)

/-- `seminorm_from_const' hf1 hc hpm 1 = 1`. -/
theorem seminorm_from_const_is_norm_one_class : seminormFromConst' hf1 hc hpm 1 = 1 :=
  by
  apply
    tendsto_nhds_unique_of_eventuallyEq (seminorm_from_const_is_limit hf1 hc hpm 1)
      tendsto_const_nhds
  simp only [Filter.EventuallyEq, Filter.eventually_atTop, ge_iff_le]
  exact ⟨1, seminormFromConstSeq_one hc hpm⟩
...
theorem seminorm_from_const_hMul (x y : R) :
  ...
  by
  have hlim :
    Filter.Tendsto (fun n => seminormFromConstSeq c f (x * y) (2 * n)) Filter.atTop
      (𝓝 (seminormFromConst' hf1 hc hpm (x * y))) :=
    by
...
theorem seminorm_from_const_neg (x : R) :
  ...
  by
  apply
    tendsto_nhds_unique_of_eventuallyEq (seminorm_from_const_is_limit hf1 hc hpm (-x))
...
theorem seminorm_from_const_add (x y : R) :
  ...
  by
  apply
    le_of_tendsto_of_tendsto' (seminorm_from_const_is_limit hf1 hc hpm (x + y))
      (Filter.Tendsto.add (seminorm_from_const_is_limit hf1 hc hpm x)
        (seminorm_from_const_is_limit hf1 hc hpm y))
  intro n
  have h_add : f ((x + y) * c ^ n) ≤ f (x * c ^ n) + f (y * c ^ n) := by rw [add_mul];
...
def seminormFromConst : RingSeminorm R
  ...
  hMul_le' := seminorm_from_const_hMul hf1 hc hpm
...
theorem seminorm_from_const_isNonarchimedean (hna : IsNonarchimedean f) :
    IsNonarchimedean (seminormFromConst' hf1 hc hpm) :=
  by
  intro x y
  apply
    le_of_tendsto_of_tendsto' (seminorm_from_const_is_limit hf1 hc hpm (x + y))
      (Filter.Tendsto.max (seminorm_from_const_is_limit hf1 hc hpm x)
        (seminorm_from_const_is_limit hf1 hc hpm y))
  intro n
  have hmax : f ((x + y) * c ^ n) ≤ max (f (x * c ^ n)) (f (y * c ^ n)) := by rw [add_mul];
    exact hna _ _
...
theorem seminorm_from_const_isPowMul : IsPowMul (seminormFromConst' hf1 hc hpm) :=
  by
  intro x m hm
  simp only [seminormFromConst']
  have hpow := Filter.Tendsto.pow (seminorm_from_const_is_limit hf1 hc hpm x) m
  have hlim :
    Filter.Tendsto (fun n => seminormFromConstSeq c f (x ^ m) (m * n)) Filter.atTop
      (𝓝 (seminormFromConst' hf1 hc hpm (x ^ m))) :=
    by
...
theorem seminorm_from_const_le_seminorm (x : R) : seminormFromConst' hf1 hc hpm x ≤ f x :=
  ...
  apply
    le_trans
      ((div_le_div_right (pow_pos (lt_of_le_of_ne (map_nonneg f c) hc) _)).mpr
...
theorem seminorm_from_const_apply_of_is_hMul {x : R} (hx : ∀ y : R, f (x * y) = f x * f y) :
  ...
  haveI hlim : Filter.Tendsto (seminormFromConstSeq c f x) Filter.atTop (𝓝 (f x)) :=
    by
    have hseq : seminormFromConstSeq c f x = fun n => f x :=
      by
      ext n
      by_cases hn : n = 0
      · simp only [seminormFromConstSeq]
...
theorem seminorm_from_const_is_hMul_of_is_hMul {x : R} (hx : ∀ y : R, f (x * y) = f x * f y)
    ...
    seminormFromConst' hf1 hc hpm (x * y) =
      seminormFromConst' hf1 hc hpm x * seminormFromConst' hf1 hc hpm y :=
  haveI hlim :
    Filter.Tendsto (seminormFromConstSeq c f (x * y)) Filter.atTop
      (𝓝 (seminormFromConst' hf1 hc hpm x * seminormFromConst' hf1 hc hpm y)) :=
    by
    rw [seminorm_from_const_apply_of_is_hMul hf1 hc hpm hx]
    have hseq : seminormFromConstSeq c f (x * y) = fun n => f x * seminormFromConstSeq c f y n :=
      by
      ext n
      simp only [seminormFromConstSeq]
...
theorem seminorm_from_const_apply_c : seminormFromConst' hf1 hc hpm c = f c :=
  haveI hlim : Filter.Tendsto (seminormFromConstSeq c f c) Filter.atTop (𝓝 (f c)) :=
    by
    have hseq : seminormFromConstSeq c f c = fun n => f c :=
      by
      ext n
      simp only [seminormFromConstSeq]
...
theorem seminorm_from_const_c_is_hMul (x : R) :
    seminormFromConst' hf1 hc hpm (c * x) =
      seminormFromConst' hf1 hc hpm c * seminormFromConst' hf1 hc hpm x :=
  by
  have hlim :
    Filter.Tendsto (fun n => seminormFromConstSeq c f x (n + 1)) Filter.atTop
      (𝓝 (seminormFromConst' hf1 hc hpm x)) :=
    by
  ...
  have hterm :
    seminormFromConstSeq c f (c * x) = fun n => f c * seminormFromConstSeq c f x (n + 1) :=
    by
    simp only [seminormFromConstSeq]
    ext n
