...
theorem tendsto_bdd_div_atTop_nhds_0_nat (f : ℕ → ℝ) (hfb : ∃ b : ℝ, ∀ᶠ n : ℕ in atTop, b ≤ f n)
...
theorem tendsto_mod_div_atTop_nhds_0_nat {m : ℕ} (hm : 0 < m) :
    Tendsto (fun n : ℕ => ((n % m : ℕ) : ℝ) / (n : ℝ)) atTop (𝓝 0) :=
  by
  apply tendsto_bdd_div_at_top_nhds_0_nat fun n : ℕ => ((n % m : ℕ) : ℝ)
  · use 0
...
theorem div_hMul_eventually_cancel (s : ℕ → ℕ) {u : ℕ → ℕ} (hu : Tendsto u atTop atTop) :
    (fun n => (s n : ℝ) / (u n : ℝ) * (u n : ℝ)) =ᶠ[atTop] fun n => (s n : ℝ) :=
  by
...
theorem Tendsto.num {s u : ℕ → ℕ} (hu : Tendsto u atTop atTop) {a : ℝ} (ha : 0 < a)
  ...
  tendsto_nat_cast_atTop_iff.mp
    (Tendsto.congr' (div_hMul_eventually_cancel s hu)
...
theorem smoothing_seminorm_seq_has_limit_aux {L : ℝ} (hL : 0 ≤ L) {ε : ℝ} (hε : 0 < ε) {m1 : ℕ}
  ...
  by
  rw [← mul_one (1 : ℝ)]
  have h_exp : tendsto (fun n : ℕ => ((n % m1 : ℕ) : ℝ) / (n : ℝ)) at_top (𝓝 0) :=
    tendsto_mod_div_at_top_nhds_0_nat hm1
  apply tendsto.mul
  · have h0 : tendsto (fun t : ℕ => -(((t % m1 : ℕ) : ℝ) / (t : ℝ))) at_top (𝓝 0) := by
...
def smoothingSeminormDef (x : R) : ℝ :=
  iInf fun n : PNat => f (x ^ (n : ℕ)) ^ (1 / (n : ℝ))

/-- If `f x = 0`, then `smoothing_seminorm_def f x` is the limit of `smoothing_seminorm_seq f x`. -/
theorem smoothingSeminormDef_is_limit_zero {x : R} (hx : f x = 0) :
    Tendsto (smoothingSeminormSeq f x) atTop (𝓝 (smoothingSeminormDef f x)) :=
  by
  ...
  simp only [hL0, smoothingSeminormSeq, smoothingSeminormDef]
...
theorem smoothingSeminormDef_is_limit_ne_zero (hf1 : f 1 ≤ 1) {x : R} (hx : f x ≠ 0) :
    Tendsto (smoothingSeminormSeq f x) atTop (𝓝 (smoothingSeminormDef f x)) :=
  by
  simp only [smoothingSeminormDef]
  set L := iInf fun n : PNat => f (x ^ (n : ℕ)) ^ (1 / (n : ℝ)) with hL
  ...
  have hL_le : L ≤ smoothingSeminormSeq f x n :=
    by
    ...
    · have hxn' : 0 < f (x ^ (n % ↑m1)) := lt_of_le_of_ne (map_nonneg _ _) (Ne.symm hxn)
      ...
      have h1 :
        ...
        rw [←
          ...
          ← rpow_mul (map_nonneg f _), ← coe_coe, one_div_mul_cancel hm10, rpow_one] at hm1 
        nth_rw 1 [← rpow_one (L + ε / 2)]
        have : (n : ℝ) / n = 1 := div_self (nat.cast_ne_zero.mpr (ne_of_gt hn0))
        nth_rw 3 [← this]; clear this
...
theorem smoothingSeminormDef_is_limit (hf1 : f 1 ≤ 1) (x : R) :
    Tendsto (smoothingSeminormSeq f x) atTop (𝓝 (smoothingSeminormDef f x)) :=
  by
  by_cases hx : f x = 0
  · exact smoothingSeminormDef_is_limit_zero f hx
  · exact smoothingSeminormDef_is_limit_ne_zero f hf1 hx

/-- If `f 1 ≤ 1`, then `smoothing_seminorm_def f x` is nonnegative. -/
theorem smoothing_seminorm_nonneg (hf1 : f 1 ≤ 1) (x : R) : 0 ≤ smoothingSeminormDef f x :=
  by
  apply ge_of_tendsto (smoothingSeminormDef_is_limit f hf1 x)
...
theorem smoothing_seminorm_zero (hf1 : f 1 ≤ 1) : smoothingSeminormDef f 0 = 0 :=
  by
  apply
    tendsto_nhds_unique_of_eventuallyEq (smoothingSeminormDef_is_limit f hf1 0) tendsto_const_nhds
  simp only [eventually_eq, eventually_at_top, ge_iff_le]
  use 1
  intro n hn
  simp only [smoothingSeminormSeq]
...
theorem smoothing_seminorm_neg (f_neg : ∀ x : R, f (-x) = f x) (x : R) :
    smoothingSeminormDef f (-x) = smoothingSeminormDef f x :=
  by
  simp only [smoothingSeminormDef, smoothingSeminormDef]
  congr; ext n
  rw [neg_pow]
...
theorem smoothing_seminorm_hMul (hf1 : f 1 ≤ 1) (x y : R) :
    smoothingSeminormDef f (x * y) ≤ smoothingSeminormDef f x * smoothingSeminormDef f y :=
  by
  apply
    le_of_tendsto_of_tendsto' (smoothingSeminormDef_is_limit f hf1 (x * y))
      (tendsto.mul (smoothingSeminormDef_is_limit f hf1 x) (smoothingSeminormDef_is_limit f hf1 y))
  intro n
...
theorem smoothing_seminorm_one (hf1 : f 1 ≤ 1) : smoothingSeminormDef f 1 ≤ 1 :=
  by
  apply le_of_tendsto (smoothingSeminormDef_is_limit f hf1 (1 : R))
  simp only [eventually_at_top, ge_iff_le]
  use 1
  rintro n hn
  simp only [smoothingSeminormSeq]
...
theorem smoothing_seminorm_le_term (x : R) (n : PNat) :
    smoothingSeminormDef f x ≤ f (x ^ (n : ℕ)) ^ (1 / n : ℝ) :=
  ciInf_le (smoothing_seminorm_seq_bdd f x) _

/-- For all `x : R`, `smoothing_seminorm_def f x ≤ f x`. -/
theorem smoothing_seminorm_le (x : R) : smoothingSeminormDef f x ≤ f x :=
  by
  convert smoothing_seminorm_le_term f x 1
  simp only [Positive.val_one, algebraMap.coe_one, coe_coe, div_one, pow_one, rpow_one]
...
def mu {x y : R}
    (hn :
      ∀ n : ℕ,
        ∃ (m : ℕ) (hm : m ∈ Finset.range (n + 1)),
          f ((x + y) ^ (n : ℕ)) ^ (1 / (n : ℝ)) ≤
            (f (x ^ m) * f (y ^ (n - m : ℕ))) ^ (1 / (n : ℝ))) :
    ℕ → ℕ := fun n => Classical.choose (hn n)

theorem mu_property {x y : R}
    (hn :
      ∀ n : ℕ,
        ∃ (m : ℕ) (hm : m ∈ Finset.range (n + 1)),
          f ((x + y) ^ (n : ℕ)) ^ (1 / (n : ℝ)) ≤ (f (x ^ m) * f (y ^ (n - m : ℕ))) ^ (1 / (n : ℝ)))
...
theorem mu_bdd {x y : R}
    (hn :
      ∀ n : ℕ,
        ∃ (m : ℕ) (hm : m ∈ Finset.range (n + 1)),
          f ((x + y) ^ (n : ℕ)) ^ (1 / (n : ℝ)) ≤ (f (x ^ m) * f (y ^ (n - m : ℕ))) ^ (1 / (n : ℝ)))
    (n : ℕ) : (mu f hn n : ℝ) / n ∈ Set.Icc (0 : ℝ) 1 :=
  by
...
private theorem f_bdd_below (s : ℕ → ℕ) {x y : R}
    (hn :
      ∀ n : ℕ,
        ∃ (m : ℕ) (hm : m ∈ Finset.range (n + 1)),
          f ((x + y) ^ (n : ℕ)) ^ (1 / (n : ℝ)) ≤ (f (x ^ m) * f (y ^ (n - m : ℕ))) ^ (1 / (n : ℝ)))
  ...
  by
  use(0 : ℝ)
  simp only [mem_lowerBounds, eventually_map, eventually_at_top, ge_iff_le, Set.mem_setOf_eq,
    forall_exists_index]
  intro r m hm
...
private theorem f_bdd_above (hf1 : f 1 ≤ 1) {s : ℕ → ℕ} (hs : ∀ n : ℕ, s n ≤ n) (x : R)
    (φ : ℕ → ℕ) : BddAbove (Set.range fun n : ℕ => f (x ^ s (φ n)) ^ (1 / (φ n : ℝ))) :=
  by
...
private theorem f_nonempty {s : ℕ → ℕ} (hs_le : ∀ n : ℕ, s n ≤ n) {x y : R}
    (hn :
      ∀ n : ℕ,
        ∃ (m : ℕ) (hm : m ∈ Finset.range (n + 1)),
          f ((x + y) ^ (n : ℕ)) ^ (1 / (n : ℝ)) ≤ (f (x ^ m) * f (y ^ (n - m : ℕ))) ^ (1 / (n : ℝ)))
    (φ : ℕ → ℕ) :
    {a : ℝ |
        ∀ᶠ n : ℝ in map (fun n : ℕ => f x ^ (↑(s (φ n)) * (1 / (φ n : ℝ)))) atTop,
          n ≤ a}.Nonempty :=
  by
  by_cases hfx : f x < 1
  · use 1
    simp only [eventually_map, eventually_at_top, ge_iff_le, Set.mem_setOf_eq]
    use 0
    intro b hb
    exact
...
private theorem f_limsup_le_one {s : ℕ → ℕ} (hs_le : ∀ n : ℕ, s n ≤ n) {x y : R}
    ...
    limsup (fun n : ℕ => f x ^ ((s (φ n) : ℝ) * (1 / (φ n : ℝ)))) atTop ≤ 1 :=
  by
  ...
  · intro c hc_bd
    ...
    · apply hc_bd (1 : ℝ) 0
      ...
      exact
        rpow_le_one (map_nonneg _ _) (le_of_lt hfx)
          (mul_nonneg (Nat.cast_nonneg _) (one_div_nonneg.mpr (Nat.cast_nonneg _)))
    · have hf_lim : tendsto (fun n : ℕ => f x ^ (↑(s (φ n)) * (1 / (φ n : ℝ)))) at_top (𝓝 1) :=
        by
        nth_rw 1 [← rpow_zero (f x)]
        refine'
          tendsto.rpow tendsto_const_nhds _
            (Or.inl (ne_of_gt (lt_of_lt_of_le zero_lt_one (not_lt.mp hfx))))
        · convert hφ_lim
          simp only [Function.comp_apply, mul_one_div]
...
theorem smoothingSeminormDef_is_limit_comp (hf1 : f 1 ≤ 1) (x : R) {φ : ℕ → ℕ}
    (hφ_mono : StrictMono φ) :
    Tendsto (fun n : ℕ => f (x ^ φ n) ^ (1 / φ n : ℝ)) atTop (𝓝 (smoothingSeminormDef f x)) :=
  haveI hφ_lim' : tendsto φ at_top at_top := StrictMono.tendsto_atTop hφ_mono
  (smoothingSeminormDef_is_limit f hf1 x).comp hφ_lim'
...
theorem limsup_mu_le (hf1 : f 1 ≤ 1) {s : ℕ → ℕ} (hs_le : ∀ n : ℕ, s n ≤ n) {x y : R}
    ...
    (hφ_lim : Tendsto ((fun n : ℕ => ↑(s n) / ↑n) ∘ φ) atTop (𝓝 a)) :
    limsup (fun n : ℕ => f (x ^ s (φ n)) ^ (1 / (φ n : ℝ))) atTop ≤ smoothingSeminormDef f x ^ a :=
  by
  by_cases ha : a = 0
  · rw [ha] at hφ_lim 
    calc
      limsup (fun n : ℕ => f (x ^ s (φ n)) ^ (1 / (φ n : ℝ))) at_top ≤
          limsup (fun n : ℕ => f x ^ ((s (φ n) : ℝ) * (1 / (φ n : ℝ)))) at_top :=
        by
        apply csInf_le_csInf
        · use(0 : ℝ)
          simp only [mem_lowerBounds, eventually_map, eventually_at_top, ge_iff_le,
            Set.mem_setOf_eq, forall_exists_index]
          intro r m hm
          exact le_trans (Real.rpow_nonneg_of_nonneg (map_nonneg f _) _) (hm m (le_refl _))
        · exact f_nonempty f hs_le hn φ
        · intro b hb
          simp only [eventually_map, eventually_at_top, ge_iff_le, Set.mem_setOf_eq] at hb ⊢
          obtain ⟨m, hm⟩ := hb
          use m
          intro k hkm
      ...
      _ = smoothingSeminormDef f x ^ a := by rw [ha, rpow_zero]
  · have ha_pos : 0 < a := lt_of_le_of_ne a_in.1 (Ne.symm ha)
    have h_eq :
      (fun n : ℕ =>
          (f (x ^ s (φ n)) ^ (1 / (s (φ n) : ℝ))) ^ ((s (φ n) : ℝ) / (φ n : ℝ))) =ᶠ[at_top]
        fun n : ℕ => f (x ^ s (φ n)) ^ (1 / (φ n : ℝ)) :=
      by
      have h : (fun n : ℕ => (1 : ℝ) / (s (φ n) : ℝ) * (s (φ n) : ℝ)) =ᶠ[at_top] 1 := by
        convert div_mul_eventually_cancel 1 (tendsto.num hφ_mono.tendsto_at_top ha_pos hφ_lim) <;>
          · ext n; simp only [Pi.one_apply, algebraMap.coe_one]
    ...
    exact
      le_of_eq
        (tendsto.limsup_eq
          (tendsto.congr' h_eq
            (tendsto.rpow
              ((smoothingSeminormDef_is_limit f hf1 x).comp
...
theorem smoothing_seminorm_isNonarchimedean (hf1 : f 1 ≤ 1) (hna : IsNonarchimedean f) :
    IsNonarchimedean (smoothingSeminormDef f) :=
  by
  intro x y
  have hn :
    ∀ n : ℕ,
  ...
  set mu : ℕ → ℕ := fun n => mu f hn n with hmu
  ...
  set b := 1 - a with hb
  have hb_lim : tendsto ((fun n : ℕ => ↑(nu n) / ↑n) ∘ φ) at_top (𝓝 b) :=
    by
    apply tendsto.congr' _ (tendsto.const_sub 1 hφ_lim)
    simp only [eventually_eq, Function.comp_apply, eventually_at_top, ge_iff_le]
    use 1
    intro m hm
    have h0 : (φ m : ℝ) ≠ 0 :=
  ...
  have hx :
    limsup (fun n : ℕ => f (x ^ mu (φ n)) ^ (1 / (φ n : ℝ))) at_top ≤
      smoothingSeminormDef f x ^ a :=
    limsup_mu_le f hf1 hmu_le hn a_in hφ_mono hφ_lim
  have hy :
    limsup (fun n : ℕ => f (y ^ nu (φ n)) ^ (1 / (φ n : ℝ))) at_top ≤
      smoothingSeminormDef f y ^ b :=
  ...
  have hxy :
    limsup (fun n : ℕ => f (x ^ mu (φ n)) ^ (1 / (φ n : ℝ)) * f (y ^ nu (φ n)) ^ (1 / (φ n : ℝ)))
        at_top ≤
      smoothingSeminormDef f x ^ a * smoothingSeminormDef f y ^ b :=
    by
    have hxy' :
      limsup (fun n : ℕ => f (x ^ mu (φ n)) ^ (1 / (φ n : ℝ)) * f (y ^ nu (φ n)) ^ (1 / (φ n : ℝ)))
          at_top ≤
    ...
    have h_bdd : is_bounded_under LE.le at_top fun n : ℕ => f (y ^ nu (φ n)) ^ (1 / (φ n : ℝ)) :=
  ...
  conv_lhs => simp only [smoothingSeminormDef]
  apply le_of_forall_sub_le
  intro ε hε
  rw [sub_le_iff_le_add]
  have h_mul :
    smoothingSeminormDef f x ^ a * smoothingSeminormDef f y ^ b + ε ≤
      max (smoothingSeminormDef f x) (smoothingSeminormDef f y) + ε :=
    by
    rw [max_def]
    split_ifs with h
    · rw [add_le_add_iff_right]
      ...
      rw [hb, ←
        rpow_add_of_nonneg (smoothing_seminorm_nonneg f hf1 _) a_in.1 (sub_nonneg.mpr a_in.2),
        add_sub, add_sub_cancel', rpow_one]
    · rw [add_le_add_iff_right]
      apply
        le_trans
  ...
  have hex :
    ...
    exists_lt_of_limsup_le
      (Real.range_bddAbove_hMul (f_bdd_above f hf1 hmu_le _ _)
          (fun n => rpow_nonneg_of_nonneg (map_nonneg _ _) _) (f_bdd_above f hf1 hnu_le _ _)
          fun n => rpow_nonneg_of_nonneg (map_nonneg _ _) _).IsBoundedUnder
      hxy hε
  obtain ⟨N, hN⟩ := hex
  apply
    le_trans
      (ciInf_le (smoothing_seminorm_seq_bdd f _)
...
def smoothingSeminorm (hf1 : f 1 ≤ 1) (hna : IsNonarchimedean f) : RingSeminorm R
  ...
  hMul_le' := smoothing_seminorm_hMul f hf1
...
theorem smoothing_seminorm_isPowMul (hf1 : f 1 ≤ 1) : IsPowMul (smoothingSeminormDef f) :=
  by
  intro x m hm
  simp only [smoothingSeminormDef]
  have hlim :
    tendsto (fun n => smoothingSeminormSeq f x (m * n)) at_top (𝓝 (smoothingSeminormDef f x)) :=
    by
    refine' tendsto.comp (smoothingSeminormDef_is_limit f hf1 x) _
  ...
  simp_rw [h_eq]
  exact smoothingSeminormDef_is_limit f hf1 _

/-- If `f 1 ≤ 1` and `∀ (1 ≤ n), f (x ^ n) = f x ^ n`, then `smoothing_seminorm_def f x = f x`. -/
theorem smoothing_seminorm_of_pow_mult (hf1 : f 1 ≤ 1) {x : R}
    (hx : ∀ (n : ℕ) (hn : 1 ≤ n), f (x ^ n) = f x ^ n) : smoothingSeminormDef f x = f x :=
  by
  apply
    tendsto_nhds_unique_of_eventuallyEq (smoothingSeminormDef_is_limit f hf1 x) tendsto_const_nhds
  simp only [eventually_eq, eventually_at_top, ge_iff_le]
  use 1
  intro n hn
  simp only [smoothingSeminormSeq]
...
theorem smoothing_seminorm_apply_of_is_mul' (hf1 : f 1 ≤ 1) {x : R}
    (hx : ∀ y : R, f (x * y) = f x * f y) : smoothingSeminormDef f x = f x :=
  by
  apply
    tendsto_nhds_unique_of_eventuallyEq (smoothingSeminormDef_is_limit f hf1 x) tendsto_const_nhds
  simp only [eventually_eq, eventually_at_top, ge_iff_le]
...
theorem smoothing_seminorm_of_mul' (hf1 : f 1 ≤ 1) {x : R} (hx : ∀ y : R, f (x * y) = f x * f y)
  ...
  by
  have hlim :
    tendsto (fun n => f x * smoothingSeminormSeq f y n) at_top
      (𝓝 (smoothingSeminormDef f x * smoothingSeminormDef f y)) :=
    by
    rw [smoothing_seminorm_apply_of_is_mul' f hf1 hx]
    exact tendsto.const_mul _ (smoothingSeminormDef_is_limit f hf1 y)
  apply tendsto_nhds_unique_of_eventuallyEq (smoothingSeminormDef_is_limit f hf1 (x * y)) hlim
  simp only [eventually_eq, eventually_at_top, ge_iff_le]
  use 1
  intro n hn1
