...
noncomputable
def LocalField.haarFunction [LocalField K] : K → ℝ := fun x ↦ MeasureTheory.Measure.addModularCharacterFun x
...
instance (K : Type*) [Field K] [NonarchLocalField K] : LocalField K :=
  NonarchLocalField.toLocalField (by infer_instance)
...
#where
example (L : Type*) [Field L] [ValuedLocalField L] : Valued L ℤₘ₀ := inferInstance
